...
def load_LR_HR_imgs_sr(fname, imsize, factor, enforse_div32=None):
    ...
    img_LR_pil = img_orig_pil.crop(bbox)
    img_LR_np = pil_to_np(img_LR_pil)
    img=np_to_pil(np.clip(img_LR_np, 0, 1))
    img.save('out/'+"LR_in_utils.jpg")
    
    # else:
        # img_HR_pil, img_HR_np = img_orig_pil, img_orig_np
        
    HR_size = [
               img_LR_pil.size[0] * factor, 
               img_LR_pil.size[1] * factor
    ]

    img_HR_pil = img_LR_pil.resize(HR_size, Image.ANTIALIAS)
    img_HR_np = pil_to_np(img_HR_pil)
    img=np_to_pil(np.clip(img_HR_np, 0, 1))
    img.save('out/'+"HR_in_util.jpg")
